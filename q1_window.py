...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        sentence = [start] * window_size + sentence + [end] * window_size #add start and end tokens
        for t in range(window_size, len(sentence) - window_size):
            window_features = []
            for w in range(-window_size,window_size+1):
                window_features.extend(sentence[t+w])
            windowed_data.append((window_features, labels[t - window_size]))
    ### END YOUR CODE
...
def test_make_windowed_data():
    ...
    sentences = [[[1,1], [2,0], [3,3], [4,4]], [[1,1], [2,0], [3,3], [4,4]]]
    sentence_labels = [[1, 2, 3, 4], [1, 2, 3, 4]]
    data = zip(sentences, sentence_labels)
    w_data = make_windowed_data(data, start=[5,0], end=[6,0], window_size=2)

    assert len(w_data) == sum(len(sentence) for sentence in sentences)

    assert w_data == [
        ([5,0] + [5,0] + [1,1] + [2,0] + [3,3], 1,),
        ([5,0] + [1,1] + [2,0] + [3,3] + [4,4], 2,),
        ([1,1] + [2,0] + [3,3] + [4,4] + [6,0], 3,),
        ([2,0] + [3,3] + [4,4] + [6,0] + [6,0], 4,),
        ([5,0] + [5,0] + [1,1] + [2,0] + [3,3], 1,),
        ([5,0] + [1,1] + [2,0] + [3,3] + [4,4], 2,),
        ([1,1] + [2,0] + [3,3] + [4,4] + [6,0], 3,),
        ([2,0] + [3,3] + [4,4] + [6,0] + [6,0], 4,),
